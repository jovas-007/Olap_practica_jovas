...
def execute_sql_file(path: str | Path, settings: Optional[AppSettings] = None) -> None:
    ...
    try:
        ...
        logger.error(
            "No se pudo conectar a la base de datos (%s). Verifica que MySQL esté "
            "en ejecución y que DATABASE_URL apunte al puerto/credenciales correctos.",
            database_url,
        )
        raise RuntimeError("Conexión a MySQL rechazada durante la ejecución SQL") from exc
