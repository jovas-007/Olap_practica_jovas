...
def safe_title(value: str) -> str:
    ...
    for token in tokens:
        ...
        if slug and token.isupper() and len(token) <= 3:
...
def assert_database_connection(settings: Optional[AppSettings] = None) -> Engine:
    """Return an engine after validating the database connection is available."""
    ...
    try:
        with engine.connect():
            logger.info("Conexión verificada contra %s", database_url)
    except OperationalError as exc:
        logger.error(
            "No se pudo conectar a la base de datos (%s). Asegúrate de que el servicio MySQL/MariaDB esté encendido, "
            "que la base exista y que el usuario tenga permisos.",
            database_url,
        )
        raise SystemExit(
            "Conexión a MySQL rechazada: inicia el servicio y revisa DATABASE_URL."
        ) from exc
    return engine
