...
def load(fact_csv: str = "data/staging/fact_ready.csv", settings: AppSettings | None = None) -> None:
    ...
    try:
        ...
        logger.error(
            "No se pudo conectar a la base de datos (%s). Asegúrate de que MySQL esté "
            "encendido y que las credenciales sean válidas.",
            database_url,
        )
        raise RuntimeError("Conexión a MySQL rechazada durante la carga de datos") from exc
