...
def _build_schedule_view(data: List[Dict[str, Any]]) -> tuple[List[Dict[str, str]], List[Dict[str, Any]]]:
    ...
    rendered: Dict[str, set[tuple[str, str, str, str, str]]] = defaultdict(set)
    ...
    while current < end_cursor:
        ...
        for day in schedule_days:
            ...
            for event in day_events:
                if event["fin"] <= current or event["inicio"] >= slot_end:
                    continue
                if not (event["inicio"] >= current and event["inicio"] < slot_end):
                    continue
                event_key = (
                    event["materia"],
                    event["clave"],
                    event["espacio"],
                    event["inicio"].strftime("%H:%M"),
                    event["fin"].strftime("%H:%M"),
                )
                if event_key in rendered[codigo]:
                    continue
                rendered[codigo].add(event_key)
                cell_events.append(
                    {
        ...
        schedule_rows.append({"dias": dia_map})
        current = slot_end

    schedule_rows = [
        row
        for row in schedule_rows
        if any(row["dias"][day["codigo"]] for day in schedule_days)
    ]
