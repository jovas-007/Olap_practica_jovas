...
def docentes_en_edificio_a_hora(edificio: str, hora_hhmm: str, usar_slots: bool = False) -> List[Dict[str, Any]]:
    ...
    for row in base_rows:
        dia_info = SETTINGS.day_map.get(row.get("dia_codigo", ""))
        if dia_info:
            row["dia"] = dia_info.nombre
        else:  # pragma: no cover - fallback for configuraciones incompletas
            row["dia"] = row.get("dia_codigo", "")
